...
#' `de_sample()` call, a [c2st()] call with no `seed` of its own -- drawing
#' from wherever the seeded call left the generator, rather than from a fresh
#' one
#' (GitHub #275, the torch analogue of #272).
#'
...
#'
#' @param seed Integer seed to set torch's global RNG to.
#' @return The RNG state captured before reseeding, suitable for
#'   [torch::torch_set_rng_state()].
#' @keywords internal
set_torch_seed <- function(seed) {
  old <- torch::torch_get_rng_state()
