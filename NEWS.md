...
* Random-number streams are now per simulation rather than per chunk, so a given seed produces the same simulations whatever the `future` plan, the worker count *and* the `chunk_size`. The 0.4.0 guarantee held only at a fixed chunk size. `chunk_size` keeps its name and its scheduling role: a chunk is the batch of draws one worker loops over, and nothing about how the simulator is called.
