...
* `sbc()` no longer emits "Chi-squared approximation may be incorrect" during ordinary use. Its per-parameter uniformity test called `stats::chisq.test(tab)` on the binned ranks, and R warns whenever a cell's expected count is low, which happens routinely at the small `n_sbc` a test suite uses to keep runtime down. Switching to `chisq.test(tab, simulate.p.value = TRUE)` computes a Monte Carlo p-value instead of relying on the asymptotic approximation, so the warning never fires; the p-value keeps the same "large = calibrated" interpretation `expected_coverage()` and existing tests rely on (#109).
