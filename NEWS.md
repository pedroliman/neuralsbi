...
* **`npe()`/`nle()` gain a `device` argument for training on a GPU.** `device = "cpu"` (the default), `"cuda"` (Linux/Windows) or `"mps"` (Apple Silicon); requesting a backend not available on this machine falls back to `"cpu"` with a warning instead of erroring, so the same call works unmodified on a laptop and a GPU box. This was previously impossible to get right without also fixing the estimators: `torch_tensor()` built from an R matrix ignores torch's default-device context, and MAF, MDN and NSF all had scratch tensors (`torch_zeros()`, an autoregressive `rev_idx`, the spline's tail padding) built without a device, so a net moved to a GPU would still hit CPU tensors partway through a forward or inverse pass. All of that is now fixed at the shared layer: `train_conditional_de()` (`R/train.R`) creates training/validation tensors on the resolved device and calls `net$to(device = )`; the estimator-specific forward/inverse code (`R/flows.R`, `R/mdn.R`, `R/nsf.R`) derives every scratch tensor from an existing tensor's device instead of relying on the default; and the `de_log_prob()`/`de_sample()` S3 methods, plus the MDN's separate i.i.d. fast path used by `nle()` (`R/likelihood.R`), move inputs to the estimator's device and results back to the CPU for the caller. A fitted estimator remembers the device it trained on (`fit$de$device`) and reuses it at `posterior()`/`sample()` time. `save_npe()` now always writes weights from the CPU regardless of training device, so a saved fit is portable across machines; `load_npe()`/`load_nle()` gain a matching `device` argument (default `"cpu"`) for choosing where to rebuild the network on reload. This PR is the portability/correctness half of GPU support only; making small networks faster on MPS specifically is a separate, lower-priority follow-up. Closes #82 (#PR_NUMBER_TODO).

# neuralsbi 0.5.9

* Internal cleanup, no user-visible behavior change: `stan_fn_lingauss()`, `stan_fn_mdn()` and `stan_fn_maf()` (`R/stan.R`) each wrote their own copy of the generated `_sum_lpdf` entry point -- the `theta` standardization, the `x` center/scale declarations, `real total = 0`, the `for (n in 1:rows(x))` loop, and the jacobian-corrected return -- differing only in the loop body and in what, if anything, gets precomputed once outside the loop (the linear-Gaussian mean/Cholesky factor and the MDN's mixture head; MAF has nothing to hoist). `R/stan.R` now carries one internal helper, `stan_sum_lines(fit, P, body, precompute = "")`, that all three call instead of repeating the block; each generator keeps only its own loop body and precompute expression inline. Generated Stan output is unchanged for all three estimators. `stan_sum_lines()` sums the same per-observation log density that `de_log_lik_iid()` (`R/likelihood.R`) sums on the R side, so a comment now points from one to the other (#64).
