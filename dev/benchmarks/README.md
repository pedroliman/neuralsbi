...
1. **SIR and Lotka-Volterra integrate their ODEs with a fixed-step vectorized RK4**, not Julia's adaptive Tsit5. Step sizes are 0.1 days for SIR and 0.0105 for Lotka-Volterra, which puts the integration error orders of magnitude below the binomial and log-normal observation noise the tasks then add.
