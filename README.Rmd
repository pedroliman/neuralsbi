...
Simulation-based inference recovers parameters from a model you can simulate but whose likelihood you would rather not write down. To make that concrete, take a noisy exponential decay measured at ten time points — a cooling object, a clearing drug — with an amplitude `a` and a decay rate `b` to recover. We know the parameters that generated the data, so we can check that the posterior lands on them.
...
```{r pairplot, fig.alt = "Pairwise posterior over the decay amplitude and rate."}
