...
device (see `net_device()`), and evaluates `log_prob_fn` under
`with_no_grad()`. `log_prob_fn` is the per-estimator tensor function –
...
pipeline stays device-agnostic plain R.
