...
abline(v = start_date + knot_days - 1, col = "firebrick", lty = 2)
```
...
straddle the breakpoint. The recovery rate \(\gamma\) is
...
n_segments <- n_knots - 1L               # 4 constant-beta segments
breakpoints <- knot_days[2:(n_knots - 1)]  # the 3 interior transition days
transition_days <- 7                     # ~1-week logistic transition per breakpoint
transition_scale <- transition_days / 4
gamma_fixed <- 1 / 7

prior <- prior_normal(
  mean = c(rep(log(0.3), n_segments), qlogis(0.1)),
  sd   = c(rep(0.5, n_segments), 1)
)

sir_rt_simulator <- function(theta, N_fixed = NULL) {
  theta <- matrix(as.numeric(theta), ncol = n_segments + 1L)
  n <- nrow(theta)
  log_beta_segments <- theta[, seq_len(n_segments), drop = FALSE]
  rho <- plogis(theta[, n_segments + 1])
  ...
  # piecewise-constant log-beta(t): each segment holds a fixed level, and a
  # logistic function blends one level into the next over a window centered
  # on the breakpoint. Weights depend only on the breakpoint days, so build
  # them once and matrix-multiply across all n draws.
  t_grid <- seq_len(days)
  w_trans <- sapply(breakpoints, function(tau) plogis((t_grid - tau) / transition_scale))
  w_ext <- cbind(1, w_trans, 0)
  W <- w_ext[, -ncol(w_ext), drop = FALSE] - w_ext[, -1, drop = FALSE]
  beta_t <- exp(log_beta_segments %*% t(W))   # n x days
...
\(\theta = (\log \beta_1, \dots, \log \beta_4, \operatorname{logit} \rho)\)
is 5-dimensional and unconstrained on \(\mathbb{R}^5\) — the log/logit
...
sbc_res    # one uniformity p-value per parameter: 4 log-beta segments, then rho
#> <nsbi_sbc> 150 trials, 300 posterior samples each
#>   per-parameter uniformity p-values (large = calibrated):
#>     0.868  0.091  0.298  0.855  0.453
```

Two parameters are worth looking at directly: \(\log \beta\) in the final
segment (day 120 — the most policy-relevant point, closest to "now" in this
window) and the ascertainment rate \(\rho\), which is the parameter most
likely to trade off against \(\beta(t)\) if the two were poorly identified.
`plot_sbc()` labels each panel generically by parameter index in `theta`, in
the order the prior and simulator use them — here that is segment 4's
\(\log \beta\) ("parameter 4") and \(\rho\) ("parameter 5").
...
<div class="figure">
<img src="figures/sir-rt-sbc-beta4-1.png" alt="SBC rank histogram for log-beta in the final (day 120) segment; a flat histogram indicates calibration."  />
<p class="caption">plot of chunk sbc-beta4</p>
</div>


``` r
plot_sbc(sbc_res, param = n_segments + 1)
```
...
draw of the 4 segment-level \(\log \beta\) values through the same
logistic-blended step function to get a full 120-day \(R(t)\) trajectory.


``` r
t_grid <- seq_len(days)
w_trans_grid <- sapply(breakpoints, function(tau) plogis((t_grid - tau) / transition_scale))
w_ext_grid <- cbind(1, w_trans_grid, 0)
W_grid <- w_ext_grid[, -ncol(w_ext_grid), drop = FALSE] - w_ext_grid[, -1, drop = FALSE]

rt_state <- function(state, n_draws = 2000) {
  x_obs <- c(log(pop[state]), log1p(case_bins[state, ]))
  draws <- sample(posterior(fit, x_obs = x_obs), n_draws)
  log_beta_t <- draws[, seq_len(n_segments)] %*% t(W_grid)   # n_draws x days
...
breakpoints even though the epidemic reached each one at a different moment;
the one-week width of the logistic transition between segments is a modeling
choice representing how quickly behavior or policy shifted, not something
the case counts separately identify; a single ascertainment rate \(\rho\)
stands in for what was, in practice, a patchwork of state and local testing
capacity; and importation is reduced to a handful of seed cases on day 1
rather than a distributed process over the following weeks. Loosening any of
these is a natural next step, and each one is a change to the simulator, not
to the inference machinery around it — the same amortized-NPE workflow
applies unchanged.
