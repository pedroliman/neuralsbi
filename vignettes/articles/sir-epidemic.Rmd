...
ggplot2::ggplot(data.frame(week = 1:Tobs, reports = x_obs),
                ggplot2::aes(x = week, y = reports)) +
  ggplot2::geom_line(colour = "steelblue") +
  ggplot2::geom_point(colour = "steelblue", size = 2) +
  ggplot2::labs(x = "week", y = "reported cases", title = "Observed outbreak") +
  ggplot2::theme_minimal()
```

<div class="figure">
<img src="figures/sir-epidemic-unnamed-chunk-4-1.svg" alt="Observed weekly reported cases from the simulated SIR outbreak."  />
...
overlay_df <- rbind(
  data.frame(beta = npe_thin[, 1], gamma = npe_thin[, 2], method = "neuralsbi (NPE)"),
  data.frame(beta = post_pomp$Beta, gamma = post_pomp$gamma, method = "pomp (pMCMC)"),
  data.frame(beta = 2, gamma = 1, method = "truth")
)
overlay_df$method <- factor(overlay_df$method,
                            levels = c("pomp (pMCMC)", "neuralsbi (NPE)", "truth"))

ggplot2::ggplot(overlay_df, ggplot2::aes(x = beta, y = gamma, colour = method,
                                         shape = method, size = method, alpha = method)) +
  ggplot2::geom_point() +
  ggplot2::scale_colour_manual(values = c("pomp (pMCMC)" = "firebrick",
                                          "neuralsbi (NPE)" = "steelblue",
                                          "truth" = "grey20")) +
  ggplot2::scale_shape_manual(values = c("pomp (pMCMC)" = 16, "neuralsbi (NPE)" = 16, "truth" = 3)) +
  ggplot2::scale_size_manual(values = c("pomp (pMCMC)" = 1, "neuralsbi (NPE)" = 1, "truth" = 3)) +
  ggplot2::scale_alpha_manual(values = c("pomp (pMCMC)" = 0.4, "neuralsbi (NPE)" = 0.4, "truth" = 1)) +
  ggplot2::coord_cartesian(xlim = c(1, 3.5), ylim = c(0.3, 1.6)) +
  ggplot2::labs(x = expression(beta), y = expression(gamma),
...
#>     Beta=0.829  gamma=0.045
plot_sbc(sbc_res, param = 1)
```

<div class="figure">
<img src="figures/sir-epidemic-unnamed-chunk-10-1.svg" alt="SBC rank histogram for beta; a flat histogram indicates calibration."  />
