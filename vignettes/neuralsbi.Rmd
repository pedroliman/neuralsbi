...
Here we write a "chain binomial" SIR model in daily time-steps, with a population of size $N$. The parameters we want to estimate are $\beta$, the recovery rate $\gamma$, and we see a fraction $\rho$ of the new infections as reported cases. In reality, we would want to inform $\gamma$ using external information, which we can do by providing an informative prior for $\gamma$.
...
It is always a good idea to test our mode, so here we simulate an outbreak to test our model:
