...
shows the workflow: a prior, a simulator,
[`npe()`](https://neuralsbi.pedrodelima.com/reference/npe.md), a
posterior. This article is about what that machinery is doing and why
anyone would want it. We use the same stochastic SIR epidemic, so the
code below will look familiar, and we spend the space on the ideas
instead.
...
evaluate its own density.
[`vignette("density-estimators")`](https://neuralsbi.pedrodelima.com/articles/density-estimators.md)
compares the alternatives.

Two consequences are worth stating plainly. First, the estimator is
trained over the prior, so it works anywhere the prior predictive
distribution puts mass, and nowhere else. Feed it an observation unlike
anything it saw in training and it will still return draws, which are
not to be trusted. Second, the fit does not depend on any particular
observation, which is what “amortized” means, and which the rest of this
article is about.
...
![Posterior R0 with 90 percent intervals against the true R0 for twelve
simulated outbreaks.](figures/intro-to-sbi-amortized-1.svg)
...
and training cost once, up front, and then condition on data for free.
It pays off whenever you have many data sets to fit, or new data
arriving, and it does not pay off when you have one observation and a
cheap likelihood.
...
![Expected-coverage curve for the fitted posterior, empirical against
nominal.](figures/intro-to-sbi-sbc-1.svg)
...
- [`vignette("density-estimators")`](https://neuralsbi.pedrodelima.com/articles/density-estimators.md)
  compares the mixture-density-network, flow and closed-form estimators.
- [`vignette("diagnostics")`](https://neuralsbi.pedrodelima.com/articles/diagnostics.md)
  is the complete diagnostic toolkit.
- [`vignette("sir-epidemic")`](https://neuralsbi.pedrodelima.com/articles/sir-epidemic.md)
  puts NPE next to `pomp`’s particle-filter MCMC on one epidemic, and
  [`vignette("sir-time-varying-beta")`](https://neuralsbi.pedrodelima.com/articles/sir-time-varying-beta.md)
  scales the idea to three competing models across all 50 US states.
