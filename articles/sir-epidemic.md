...
systems that show up all over epidemiology and ecology. It fits them by
particle filtering: a sequential Monte Carlo scheme that only ever
simulates the latent state process, so you never write down its
transition density. That makes `pomp` simulation-based in the same
spirit as `neuralsbi`. The two packages differ in what they do next, and
...
snippets and `neuralsbi` wants a plain R function. The dynamics are
...
curve is the data both methods condition on.
...
Metropolis-Hastings sampler. We put uniform priors on the two rates, the
same priors `neuralsbi` will use, and run a short adaptive chain.
...
The chain concentrates around the true rates. This posterior belongs to
...
two-sample test. An accuracy near 0.5 means a classifier cannot tell the
two sets of draws apart.
...
The ranks are flat and the p-values are large, so NPE’s wider intervals
are calibrated rather than broken. The two packages give the same answer
here, and NPE reports a touch more uncertainty for the simulations it
was given.
...
Use `pomp` when you can write the measurement density and have one or a
few data sets to fit. It will give you the sharpest and best understood
posterior. Use `neuralsbi` when the measurement model itself is a black
box, or when you need to condition on many data sets and amortization
