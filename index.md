...
in this example that the quantity that is better identified by the model
is `R0 = beta / gamma`, but that is fine. In a real-world application,
we would constrain `gamma` given external information.
...
`gamma = 1/7`. Note that our neural posterior estimator *hasn’t seen any
data yet*. We get a posterior by calling the `posterior` function on our
`npe` and giving it a data vector. The `posterior` call is fast.
